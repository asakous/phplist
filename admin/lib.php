...
function shortenTextDisplay($text,$max = 30) {
  ...
  if (mb_strlen($text) > $max) {
    if ($max < 30) {
      $display = word_cut($text,0,$max - 4).' ... ';
    } else {
      $display = word_cut($text,0,20).' ... '.word_cut($text,-10);
	  // $display = word_cut($text,30);
    }
...
##http://stackoverflow.com/questions/4451664/make-php-pathinfo-return-the-correct-filename-if-the-filename-is-utf-8
